...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    //compute pressure of atom i with Tait EOS
     ...
     //compute eos of atom i with isothermal
    
     fi = rho0[itype] + soundspeed[itype]*soundspeed[itype]*(rho0[itype] - rho[i]);
     
     //compute eos with graham correction(atom i)
     
    fi = ((soundspeed[itype]*soundspeed[itype])*rho0[itype])/rho[i]((fi/rho0[itype])^rho[i] - 1);
	...
	//compute pressure of atom j with isothermal
	
        fj = rho0[jtype] + soundspeed[jtype]*soundspeed[jtype]*(rho0[jtype] - rho[j]);
	
	
	 //compute eos with graham correction(atom j)
     
    fj = ((soundspeed[jtype]*soundspeed[jtype])*rho0[jtype])/rho[j]((fi/rho0[jtype])^rho[j] - 1);
